...
הפרויקט עבר משלב MVP ראשוני לשלב מאגר תוכן פעיל ובטא ציבורית זהירה.

- יש 26 בדיקות שפורסמו או מוכנות לפרסום בענף הנוכחי.
- יעד קצר ראשון של 24 בדיקות הושלם.
- בהמשך, ספירת הבדיקות הציבורית צריכה להיגזר מרשומות התוכן ב-`content/claims/` ומהאתר החי, לא מספירה ידנית בדוקומנטציה.
- השלמת יעד התוכן הראשוני אינה סיום מותג או מוצר מלא.
