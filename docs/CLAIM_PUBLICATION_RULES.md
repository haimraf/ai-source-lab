...
חשוב: `updateClaimSitemapXml` שומר בכוונה את ה-origin שכבר קיים בקובץ ה-sitemap. כלומר אם fixture בטסט משתמש ב-`https://example.com`, התוצאה אמורה להישאר עם `https://example.com`. לא מתקנים את הפונקציה ל-domain של הפרודקשן במקרה כזה; מתקנים את הציפייה בטסט. ה-domain האמיתי צריך להגיע מה-sitemap האמיתי, לא מהנחה בתוך הטסט.

אם הטסט נופל על הודעת שגיאה מדויקת, לבדוק קודם מה ההודעה הנוכחית בקוד לפני שמשנים פונקציה עובדת.
