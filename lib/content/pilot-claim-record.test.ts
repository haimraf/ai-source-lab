...
describe("pilot claim content record", () => {
  it("defines the AI-as-source claim as complete typed static content", () => {
    ...
    expect(typedPilotClaim.body?.length).toBeGreaterThan(0);
  });
  ...
  it("preserves the static page body order and cutover metadata", () => {
    ...
    expect(typedPilotClaim.body?.map((block) => block.type)).toEqual([
      "answer-box", "verdict-grid", "evidence-list", "paragraph-section", "comparison-list", "method-note",
      "logic-chain", "comparison-list", "faq", "copy-share", "sources", "method-note",
    ]);
    expect(typedPilotClaim.body).toEqual(expect.arrayContaining([
  ...
  it("does not create or remove public page routes", () => {
    ...
    expect(existsSync("app/claims/15-minute-city-prison/page.tsx")).toBe(true);
  });
