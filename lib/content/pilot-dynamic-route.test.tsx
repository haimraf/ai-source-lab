...
const otherStaticClaimPages = ["15-minute-city-prison", "agenda-2030-seven-steps", "who-pandemic-agreement-sovereignty"] as const;
...
describe("dynamic claim route cutovers", () => {
  ...
  it("moves only the approved pages to dynamic route ownership", () => {
    ...
    for (const slug of [pilotClaim.slug, gatewayClaim.slug, blueBeamClaim.slug, cloudClaim.slug, chemtrailsClaim.slug, xrpClaim.slug, aiBciClaim.slug]) {
      expect(existsSync(`app/claims/${slug}/page.tsx`), `${slug} should be dynamic`).toBe(false);
    }
